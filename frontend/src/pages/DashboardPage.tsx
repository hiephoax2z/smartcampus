GIT_AUTHOR_DATE="2026-04-28T16:00:00" GIT_COMMITTER_DATE="2026-04-28T16:00:00" git commit -m "feat: build student dashboard"
...
function DashboardEnseignant() {
  ...
  const urgentEval: EvaluationAVenir | null = evaluationsAVenir.reduce((max: EvaluationAVenir | null, ev: EvaluationAVenir) => {
      ...
      <div style={{ padding: '2rem 2.5rem', maxWidth: 1400 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={card}>
              <h3 style={{ color: '#fff', fontSize: 14, fontWeight: 600, marginBottom: '1rem' }}>
                Aujourd'hui <span style={{ marginLeft: 8, color: '#444', fontSize: 12, fontWeight: 400 }}>{todaySessions.length} séance{todaySessions.length !== 1 ? 's' : ''}</span>
              </h3>
              {todaySessions.length === 0 ? <p style={{ color: '#444', fontSize: 13 }}>Aucune séance prévue aujourd'hui.</p> : (
            ...
            <div style={card}>
              ...
              {evaluationsAVenir.length === 0 ? <p style={{ color: '#444', fontSize: 13 }}>Tout est à jour.</p> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
...
function getSessionStatus(heure_debut: string, heure_fin: string) {
  ...
  const m    = diff % 60
  const label = h > 0 ? `DANS ${h}H${m > 0 ? String(m).padStart(2, '0') : ''}` : `DANS ${m}MIN`
...
function DashboardEtudiant() {
  ...
  const moyenneNum = notesAvecVal.length > 0 ? notesAvecVal.reduce((sum: number, n: NoteRecente) => sum + (n.note ?? 0), 0) / notesAvecVal.length : null
  ...
  const th: React.CSSProperties = { textAlign: 'left', padding: '0 0 10px', fontSize: 11, color: '#444', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 600, borderBottom: '1px solid #1f1f1f' }
  const td: React.CSSProperties = { padding: '10px 0', fontSize: 13, verticalAlign: 'middle', borderBottom: '1px solid #161616', color: '#888' }
  return (
    <div style={{ background: '#111111', minHeight: '100%' }}>
      <div style={{ padding: '2rem 2.5rem', maxWidth: 1400 }}>
        <div style={{ marginBottom: '2rem' }}>
          <p style={{ color: '#555', fontSize: '11px', letterSpacing: '0.18em', marginBottom: '8px' }}>TABLEAU DE BORD</p>
          <h1 style={{ fontFamily: SERIF, fontSize: 'clamp(2rem, 4vw, 3rem)', color: '#fff', lineHeight: 1.1, fontWeight: 700 }}>
            Bonjour,{' '}<span style={{ color: ACCENT, fontStyle: 'italic' }}>{user?.prenom}.</span>
          </h1>
          <p style={{ color: '#555', marginTop: '0.5rem', fontSize: '13px' }}>Voici un résumé de votre semaine académique.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
          <DarkStatCard label="Moyenne générale" value={moyenneNum !== null ? `${moyenneNum.toFixed(1)}` : '—'} unit={moyenneNum !== null ? '/20' : ''} sub="ce semestre" accent />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '1.5rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <DarkPanel title="Aujourd'hui" subtitle={JOURS_FR[jourIdx].charAt(0).toUpperCase() + JOURS_FR[jourIdx].slice(1)}>
              {todaySessions.length === 0 ? <p style={{ color: '#444', fontSize: '13px', padding: '0.75rem 0' }}>Aucune séance prévue aujourd'hui.</p> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {todaySessions.map((s, i) => {
                    const status = getSessionStatus(s.heure_debut, s.heure_fin)
                    const tc     = TYPE_COLORS[s.type] ?? TYPE_COLORS.cours
                    return (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.875rem 1rem', borderRadius: 12, background: '#161616', border: '1px solid #1f1f1f' }}>
                        <div style={{ width: 4, height: 44, borderRadius: 2, background: tc.border, flexShrink: 0 }} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ color: '#fff', fontSize: '14px', fontWeight: 600, marginBottom: 2 }}>{s.cours_nom}</p>
                          <p style={{ color: '#666', fontSize: '12px' }}>{s.heure_debut.slice(0, 5)} – {s.heure_fin.slice(0, 5)}{s.salle_nom ? ` · ${s.salle_nom}` : ''}</p>
                        </div>
                        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', padding: '2px 8px', borderRadius: 4, background: tc.bg, color: tc.text, border: `1px solid ${tc.border}22` }}>{TYPE_LABEL[s.type] ?? s.type.toUpperCase()}</span>
                        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', padding: '4px 10px', borderRadius: 20, background: status.bg, color: status.color, flexShrink: 0 }}>{status.label}</span>
                      </div>
                    )
                  })}
                </div>
            ...
            <DarkPanel title="Notes récemment publiées">
              {notesRecentes.length === 0 ? <p style={{ color: '#444', fontSize: '13px', padding: '0.75rem 0' }}>Aucune note disponible.</p> : (
                  ...
                  <tbody>
                    {(notesRecentes as NoteRecente[]).map((n, i) => (
                      <tr key={i}>
                        ...
                        <td style={td}><span style={{ fontFamily: 'monospace', fontSize: 11, color: ACCENT }}>{n.cours_code}</span></td>
                        <td style={td}><span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: '#1a1a1a', color: '#666', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{n.evaluation_type}</span></td>
                        <td style={{ ...td, textAlign: 'right' }}><NoteDisplay note={n.note} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div>
            <DarkPanel title="Boîte de réception" subtitle={`${unread} non lue${unread !== 1 ? 's' : ''}`}>
              {notifList.length === 0 ? <p style={{ color: '#444', fontSize: '13px', padding: '0.75rem 0' }}>Aucune notification.</p> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {notifList.slice(0, 8).map((n, i) => (
                    <div key={i} style={{ padding: '0.75rem', borderRadius: 10, background: n.lu ? 'transparent' : '#161616', border: `1px solid ${n.lu ? '#181818' : '#222'}` }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                        {!n.lu && <div style={{ width: 6, height: 6, borderRadius: '50%', background: ACCENT, marginTop: 5, flexShrink: 0 }} />}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ color: n.lu ? '#555' : '#bbb', fontSize: 13, lineHeight: 1.45, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const, overflow: 'hidden' }}>{n.message}</p>
                          <p style={{ color: '#3a3a3a', fontSize: 11, marginTop: 4 }}>{formatDate(n.created_at)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
...
function StatCardLight({ label, value, color }: { label: string; value: number; color: string }) {
  const colors: Record<string, string> = { blue: 'bg-blue-50 text-blue-700', violet: 'bg-violet-50 text-violet-700', emerald: 'bg-emerald-50 text-emerald-700', amber: 'bg-amber-50 text-amber-700' }
  return (
...
function CapaciteBadge({ current, max }: { current: number; max: number }) {
  ...
  return <span className={`text-xs font-medium px-2 py-0.5 rounded shrink-0 ${color}`}>{current}/{max}</span>
}

function Avatar({ name }: { name: string }) {
  const initials = name.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase()
  return <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold shrink-0">{initials}</div>
}

function SeanceCardLight({ seance }: { seance: Seance }) {
  const typeColors: Record<string, string> = { cours: 'bg-blue-50 text-blue-700', td: 'bg-violet-50 text-violet-700', tp: 'bg-emerald-50 text-emerald-700', examen: 'bg-red-50 text-red-700' }
  return (
    <div className="border border-gray-100 rounded-lg p-3 space-y-1">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${typeColors[seance.type] ?? 'bg-gray-100 text-gray-600'}`}>{TYPE_LABEL[seance.type] ?? seance.type}</span>
      </div>
